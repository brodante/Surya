...
void newfile(string &backup)
 ...
 void oldfile()
  {
    sting a;
    cout<<"Enter the name (with extension) of the file you want to open. /n/n/t";
    cin>>a;
    ifstream file;
    file.open(a);
    getline(file,backup);
...
string decryptv2()
  {
    ifstream file;
    file.open("encrypted_v.1.00.txt", ios_base::app);
    unbuffer
    getline(file,backup);
    file.close();
...
void save(string &backup)
  {
    ofstream file;
    file.open("encrypted_v.1.00.txt");
    file<<backup;
    file.close();
  }
